...
def setup_distributed(
    ...
    try:
        ...
        # Lấy thông tin về GPU đó
        # torch.distributed.init_process_group(backend=backend, init_method='env://')
        torch.distributed.init_process_group(init_method="env://") # Khởi tạo quá trình phân tán kết nối các GPU với nhau
        torch.distributed.barrier() # Chờ tất cả các tiến trình kết nối xong

        rank = torch.distributed.get_rank() # Lấy rank (ID) của GPU hiện tại
        torch.cuda.set_device(rank) # Thiết lập GPU hiện tại dựa trên rank
        torch.cuda.empty_cache() # Giải phóng bộ nhớ GPU không sử dụng
        enabled_dist = True
